import type { CalendarEvent, Conversion } from "../../lib/ipc";
import { gregorianLongText, isSaturday, nepaliLongText } from "../../lib/dayDetail";
import type { NumeralStyle } from "../../lib/numerals";
import { useSettings } from "../../lib/settings";
import { Icon } from "../Icon";
import { CompactCopyRow } from "./CompactCopyRow";
...
export function DateSummaryPanel({
  ...
  return (
    <section className="surface-card day-summary p-3">
      ...
      <p className="mt-0.5 text-[12px] text-text-secondary">{gregorianLongText(conversion.gregorian)}</p>
          ...
          <div className="space-y-1.5">
            {event?.name && (
              <div className="flex items-start gap-1.5">
                ...
                <p className={`text-[13px] leading-snug ${holiday ? "text-holiday" : ""}`}>{event.name}</p>
              </div>
            )}
            {event?.tithi && (
            ...
            {holiday && (
              <div className="flex items-center gap-1.5 pl-0.5">
                ...
                <span className="text-[10px] font-medium text-holiday">{t("calendar.public-holiday")}</span>
              </div>
            )}
          </div>
        </>
