...
import { api, type DayPlan, type NepaliDate } from "../../lib/ipc";
    ...
    <div className="group flex items-start gap-1.5 py-1.5">
      <button type="button" onClick={onEdit} className="min-w-0 flex-1 text-left">
        <p className="truncate text-[12px]">
          {time && <span className="mr-1.5 font-semibold tabular-nums text-accent-mark">{time}</span>}
          <span className="font-medium">{plan.title}</span>
        </p>
        {plan.note && <p className="mt-0.5 line-clamp-2 text-[11px] text-text-muted">{plan.note}</p>}
