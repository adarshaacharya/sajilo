...
import type { Panchanga } from "../../lib/ipc";
  ...
  return (
    <section className="surface-card p-3">
      ...
      <div className="flex gap-2">
        <Reading icon="sunrise" label={t("panchanga.sunrise")} value={clockTime(panchanga.sunrise)} />
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0">
              <p className="text-[12px] font-medium tabular-nums">
                {t("panchanga.rahu-kaal")}{" "}
                {clockTime(panchanga.rahuKaalStart)}–{clockTime(panchanga.rahuKaalEnd)}
              </p>
              <p className="mt-0.5 text-[10px] leading-snug text-text-muted">{t("panchanga.rahu-note")}</p>
            </div>
          </div>
        </>
      )}
