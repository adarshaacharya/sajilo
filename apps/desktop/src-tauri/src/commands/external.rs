//! Opens trusted web links in the user's browser.

/// Opens an HTTP(S) URL with the platform's default browser.
///
...
mod tests {
    ...
    async fn rejects_non_web_urls() {
        ...
        assert_eq!(
            result,
            Err("only HTTP and HTTPS links can be opened".to_owned())
        );
    }
}
